...
const updateVideo = async (req, res, next) => {
  try {
    ...
    if (req.user.id === video.userId) {
      ...
      return next(
        createError(404, "Put these foolish ambitions of updating to rest!")
      );
    }
  } catch (err) {
    next(err);
...
const deleteVideo = async (req, res, next) => {
  try {
    ...
    if (req.user.id === video.userId) {
      ...
      res.status(200).json("Video Banished");
    } else {
      return next(
        createError(404, "Put these foolish ambitions of deleting to rest!")
      );
    }
  } catch (err) {
    next(err);
